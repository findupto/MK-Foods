...
#[tauri::command] fn save_product(w:WebviewWindow,s:State<Mutex<AppState>>,p:Value)->Result<Value,String>{let mut state=s.lock().unwrap();ensure(&mut state,&w,Some(&["Admin","Owner"]))?;let id=p["id"].as_str().filter(|x|!x.is_empty()).map(str::to_string).unwrap_or_else(||format!("p-{}",now_ms()));let name=p["name"].as_str().unwrap_or("").trim().to_string();if name.is_empty(){return Err("Product name is required".into())};let x=json!({"id":id,"name":name,"category":p["category"].as_str().unwrap_or("General").trim(),"price":p["price"].as_f64().unwrap_or(0),"available":p["available"].as_bool().unwrap_or(true)});let mut updated=false;for q in array_mut(&mut state.db,"products"){if q["id"].as_str()==Some(&id){*q=x.clone();updated=true;break}}if !updated{array_mut(&mut state.db,"products").push(x.clone())};audit(&mut state,if updated{"MENU_UPDATED"}else{"MENU_CREATED"},json!({"id":id}))?;Ok(x)}
