...
#[cfg(windows)] unsafe fn from_wide_ptr(p: *const u16) -> String { if p.is_null() { return String::new(); } let mut n = 0usize; while *p.add(n) != 0 { n += 1; } String::from_utf16_lossy(std::slice::from_raw_parts(p, n)) }

#[cfg(windows)] unsafe fn enumerate_printers() -> Result<Vec<Value>, String> {
    ...
    if EnumPrintersW(flags, std::ptr::null(), 4, buffer.as_mut_ptr(), needed, &mut needed, &mut returned) == 0 { return Err(format!("PRINTER_ENUMERATION_FAILED: {}", std::io::Error::last_os_error())); }
    let rows = std::slice::from_raw_parts(buffer.as_ptr() as *const PrinterInfo4W, returned as usize);
    let mut out: Vec<Value> = Vec::with_capacity(rows.len());
    for row in rows {
        let name = from_wide_ptr(row.printer_name); let server = from_wide_ptr(row.server_name);
        if name.trim().is_empty() { continue; }
        let mut name_w = wide(&name); let mut handle: Handle = std::ptr::null_mut();
        let online = OpenPrinterW(name_w.as_mut_ptr(), &mut handle, std::ptr::null_mut()) != 0 && !handle.is_null();
        if online { ClosePrinter(handle); }
        out.push(json!({"name":name,"server":server,"connection":if name.starts_with("\\\\"){"network"}else{"windows-raw"},"online":online,"status":if online{"Connected"}else{"Offline"}}));
    }
    out.sort_by(|a,b| a["name"].as_str().unwrap_or("").to_ascii_lowercase().cmp(&b["name"].as_str().unwrap_or("").to_ascii_lowercase())); Ok(out)
}
#[cfg(not(windows))] fn enumerate_printers() -> Result<Vec<Value>, String> { Ok(Vec::new()) }
...
#[cfg(windows)] fn bluetooth_mac_from_instance(i:&str)->String { let u=i.to_ascii_uppercase(); for m in ["DEV_","ADDR_"] { if let Some(p)=u.find(m) { let t=&u[p+m.len()..]; let h:String=t.chars().take_while(|c|c.is_ascii_hexdigit()).collect(); let n=normalize_mac(&h); if !n.is_empty(){return n} } } String::new() }
...
#[cfg(windows)] fn enumerate_bluetooth_devices()->Result<Vec<Value>,String>{
    ...
    for x in ds { let inst=x["InstanceId"].as_str().unwrap_or(""); let mac=bluetooth_mac_from_instance(inst); let status=x["Status"].as_str().unwrap_or("Unknown"); let name=x["FriendlyName"].as_str().unwrap_or("Bluetooth device").trim(); if name.is_empty(){continue} if r.iter().any(|d|d["name"].as_str().unwrap_or("").eq_ignore_ascii_case(name)&&d["mac"].as_str().unwrap_or("").eq_ignore_ascii_case(&mac)){continue} r.push(json!({"name":name,"status":status,"instanceId":inst,"mac":mac,"class":x["ClassName"].as_str().unwrap_or(""),"connection":"bluetooth-pnp","discoveryMethod":"Windows PnP / BTHENUM","paired":true,"online":status.eq_ignore_ascii_case("OK"),"thermalCandidate":likely_thermal_name(name),"methods":["windows-spooler","bluetooth-com","bluetooth-spp","bluetooth-niimbot-ble"]})); }
...
#[tauri::command] pub fn discover_printers(window:WebviewWindow,state:State<Mutex<AppState>>)->Result<Value,String>{require_session(&state,&window)?;#[cfg(windows)]{return Ok(json!({"ok":true,"printers":unsafe{enumerate_printers()?}}));}#[cfg(not(windows))]{Ok(json!({"ok":true,"printers":[]}))}}
#[tauri::command] pub fn connect_printer(window:WebviewWindow,state:State<Mutex<AppState>>,name:String)->Result<Value,String>{require_session(&state,&window)?;#[cfg(windows)]{let mut printer=wide(&name);let mut handle:Handle=std::ptr::null_mut();let ok=unsafe{OpenPrinterW(printer.as_mut_ptr(),&mut handle,std::ptr::null_mut())}!=0&&!handle.is_null();if ok{unsafe{ClosePrinter(handle)};return Ok(json!({"ok":true,"name":name,"connection":"windows-raw"}))}return Err(format!("PRINTER_OPEN_FAILED: {}",std::io::Error::last_os_error()));}#[cfg(not(windows))]{Err("WINDOWS_PRINTER_ONLY".into())}}

#[tauri::command] pub fn print_thermal(window:WebviewWindow,state:State<Mutex<AppState>>,printer:String,data:Vec<u8>)->Result<Value,String>{require_session(&state,&window)?;if printer=="__BLUETOOTH_DISCOVER__"{return Ok(json!({"ok":true,"devices":enumerate_bluetooth_devices()?}))}if printer=="__DISCOVER__"{return Ok(json!({"ok":true,"printers":enumerate_printers()?}))}if printer.starts_with("__BLUETOOTH_RAW__|"){let mac=printer.split_once('|').map(|(_,v)|v).unwrap_or("");let written=print_bluetooth_spp(mac,&data)?;return Ok(json!({"ok":true,"written":written,"route":"bluetooth-spp"}))}if printer.starts_with("__BLUETOOTH_COM__|"){let com=printer.split_once('|').map(|(_,v)|v).unwrap_or("");let written=print_bluetooth_com(com,&data)?;return Ok(json!({"ok":true,"written":written,"route":"bluetooth-com"}))}if printer.starts_with("__NETWORK_RAW__|"){let target=printer.split_once('|').map(|(_,v)|v).unwrap_or("");let written=print_network_raw(target,&data)?;return Ok(json!({"ok":true,"written":written,"route":"network-raw"}))}#[cfg(windows)]{let written=unsafe{print_windows_raw(&printer,&data)?};return Ok(json!({"ok":true,"written":written,"route":"windows-raw","printer":printer}))}#[cfg(not(windows))]{Err("WINDOWS_PRINTER_ONLY".into())}}
