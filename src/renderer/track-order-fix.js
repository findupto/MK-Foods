(() => {
  ...
  const moneySafe = value => {
    try { return typeof money === 'function' ? money(Number(value || 0)) : `Rs. ${Number(value || 0).toLocaleString()}`; }
    catch (_) { return `Rs. ${Number(value || 0).toLocaleString()}`; }
  };
  ...
  function closeTrackModal() {
    document.getElementById('mkTrackOrderModal')?.remove();
  }
  ...
  function openTrackModal(orderId) {
    ...
    if (!order) {
      ...
      else console.warn('[MK Foods] Track Order: order not found', orderId);
      return false;
    }
    ...
    const events = [...audit.map(e => ({
      at: e.at || e.createdAt || order.createdAt,
      action: e.action || e.status || 'Order update',
      user: e.by || e.user || e.username || '-'
    })), ...workflowEvents.map(e => ({
      at: e.at || order.createdAt,
      action: e.action || 'Workflow update',
      user: e.user || e.by || '-'
    }))]
      .filter(e => e.at)
      .sort((a, b) => new Date(a.at) - new Date(b.at));

    const kitchens = Object.entries(workflow.assignments || {}).map(([name, data]) => `
      <div class="mk-track-kitchen">
        <div><b>${esc(name)}</b><span class="tag">${esc(data?.status || 'pending')}</span></div>
        <small>Forwarded: ${esc(data?.forwardedBy || '-')} · Prepared: ${esc(data?.preparedBy || data?.cookedBy || '-')} · Ready: ${esc(data?.readyBy || '-')}</small>
      </div>`).join('');

    const timeline = events.length ? events.map(e => `
      <div class="mk-track-event">
        <span class="mk-track-dot"></span>
        <div><b>${esc(String(e.action).replaceAll('_', ' '))}</b><small>${esc(e.user)} · ${esc(new Date(e.at).toLocaleString())}</small></div>
      </div>`).join('') : '<div class="muted">No workflow events recorded yet.</div>';

    closeTrackModal();
    const root = document.createElement('div');
    root.id = 'mkTrackOrderModal';
    root.className = 'detail-modal open';
    root.innerHTML = `
      <div class="detail-backdrop" data-track-close></div>
      <div class="detail-dialog" role="dialog" aria-modal="true" aria-label="Order Tracking">
        <div class="detail-head">
          <h2>Order Tracking · ${esc(order.id)}</h2>
          <button class="mini" type="button" data-track-close>Close</button>
        </div>
        <div class="detail-grid">
          <div class="detail-kv"><span>Order</span><b>${esc(order.id)}</b></div>
          <div class="detail-kv"><span>Type</span><b>${esc(order.orderType || '-')}</b></div>
          <div class="detail-kv"><span>Status</span><b>${esc(order.status || workflow.stage || '-')}</b></div>
          <div class="detail-kv"><span>Workflow stage</span><b>${esc(workflow.stage || order.workflowStatus || '-')}</b></div>
          <div class="detail-kv"><span>Customer</span><b>${esc(order.customerName || 'Walk-in')}</b></div>
          <div class="detail-kv"><span>Total</span><b>${moneySafe(order.total)}</b></div>
          <div class="detail-kv"><span>Payment</span><b>${esc(order.payment || '-')} · ${esc(order.paymentStatus || '-')}</b></div>
          <div class="detail-kv"><span>Created</span><b>${esc(new Date(order.createdAt || Date.now()).toLocaleString())}</b></div>
        </div>
        <h3>Kitchen Tracking</h3>
        <div class="mk-track-kitchens">${kitchens || '<div class="muted">No kitchen assignment recorded.</div>'}</div>
        <h3>Timeline</h3>
        <div class="mk-track-timeline">${timeline}</div>
      </div>`;
  ...
  document.addEventListener('click', event => {
    ...
    if (!/^(track|track order|track\s+details)$/i.test(label)) return;
