(() => {
  ...
  const defaults={recipes:{}, ingredients:{}, units:{}, suppliersLedger:[], payments:[], promotions:[], stations:{}, printers:{}, branches:[], approvals:[], notifications:[], reservations:[], deliveryZones:[], qr:{enabled:true}, sync:{queue:[],lastPush:null}, backups:[], settings:{autoBackup:true,backupIntervalHours:24}};
  ...
  const transitions={draft:['held','confirmed','cancelled'],held:['draft','confirmed','cancelled'],confirmed:['sent_to_kitchen','cancelled','voided'],sent_to_kitchen:['preparing','held','voided'],preparing:['ready','held','voided'],ready:['collecting','dispatching','completed'],collecting:['completed'],dispatching:['out_for_delivery','completed'],out_for_delivery:['completed','returned'],completed:['refunded','reopened'],reopened:['confirmed','cancelled'],cancelled:[],voided:[],refunded:[],returned:[]};
  window.mkProduction=window.mkProduction||{};
  window.mkProduction.state=()=>clone(state);
  window.mkProduction.transition=(order,next,reason='')=>{const cur=order.status==='new'?'draft':(order.status||'draft');if(!(transitions[cur]||[]).includes(next))throw new Error(`Invalid order transition: ${cur} → ${next}`);if(['voided','refunded','reopened'].includes(next)&&!manager())throw new Error('MANAGER_APPROVAL_REQUIRED');order.status=next;order.statusAt=new Date().toISOString();order.statusReason=reason;audit('ORDER_STATUS_CHANGED',{orderId:order.id,from:cur,to:next,reason});return order};

  window.mkProduction.saveIngredient=(x)=>{if(!x?.name)throw Error('Ingredient name required');const ingredientId=x.id||id('ING');state.ingredients[ingredientId]=Object.assign({id:ingredientId,unit:'g',costPerUnit:0,stock:0,minStock:0},x,{id:ingredientId});save();return state.ingredients[ingredientId]};
  ...
  const factor=(from,to)=>{if(from===to)return 1;const m={mg:.001,g:1,kg:1000,ml:1,l:1000,each:1,box:1};if(m[from]!=null&&m[to]!=null)return m[from]/m[to];throw Error(`UNIT_CONVERSION_UNSUPPORTED:${from}->${to}`)};
  window.mkProduction.recipeCost=(productId)=>((state.recipes[productId]||[]).reduce((s,l)=>{const i=state.ingredients[l.ingredientId]||{};return s+Number(l.qty||0)*factor(l.unit||i.unit||'each',i.costUnit||i.unit||'each')*Number(i.costPerUnit||0)},0));
  window.mkProduction.consumeRecipe=(productId,multiplier=1,reason='SALE')=>{const changes=[];(state.recipes[productId]||[]).forEach(l=>{const i=state.ingredients[l.ingredientId];if(i){const used=Number(l.qty||0)*Number(multiplier||0)*factor(l.unit||i.unit||'each',i.unit||'each');i.stock=Math.max(0,Number(i.stock||0)-used);changes.push({ingredientId:i.id,used,remaining:i.stock,reason});if(i.stock<=Number(i.minStock||0))notify('low-stock','Low ingredient stock',`${i.name} is below minimum.`)}});save();return changes};
  ...
  const oldCreate=window.mkFoods?.createOrder;
  if(oldCreate&&!oldCreate.__recipeWrapped){const wrapped=async order=>{const result=await oldCreate(order);if(result?.ok===false)return result;for(const item of order?.items||[])if(state.recipes[item.id])window.mkProduction.consumeRecipe(item.id,Number(item.qty||0),'SALE');window.mkProduction.queueSync('ORDER_CREATED',{orderId:order?.id});return result};wrapped.__recipeWrapped=true;window.mkFoods.createOrder=wrapped}
