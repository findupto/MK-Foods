(() => {
  ...
  window.mkFoods={
    ...
    changePassword:(current,next)=>safe('change_password',{current,next}),resetPassword:(target,temp)=>safe('reset_password',{target,temp}),createOrder:order=>safe('create_order',{o:order}),orderStatus:(id,status)=>safe('order_status',{id,status}),assignOrder:(id,rider)=>safe('assign_order',{id,rider}),updateTable:(id,status)=>safe('update_table',{id,status}),
    ...
    saveProduct:p=>safe('save_product',{p}),deleteProduct:id=>safe('delete_product',{id}),replaceProducts:p=>safe('replace_products',{products:p}),updateSettings:s=>safe('update_settings',{settings:s}),discoverPrinters:()=>safe('discover_printers'),discoverBluetooth:async()=>{const r=await safe('print_thermal',{printer:'__BLUETOOTH_DISCOVER__',data:[]});return r?.devices?{ok:true,devices:r.devices}:r},connectPrinter:name=>safe('connect_printer',{name}),
    printThermal:(printer,data)=>safe('print_thermal',{printer,data:Array.from(data instanceof Uint8Array?data:new Uint8Array(data))}),
