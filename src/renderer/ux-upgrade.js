(() => {
  ...
  function ensureUi() {
    ...
    style.id = 'mkUxStyle';
    style.textContent = `
      #mkUxLayer{position:fixed;inset:68px 0 0 205px;z-index:99999;display:none;align-items:stretch;justify-content:flex-end;background:var(--mk-bg,#f5f7fb)}
      #mkUxLayer.open{display:flex}.mkux-dialog{width:min(760px,100%);height:100%;overflow:hidden;display:flex;flex-direction:column;background:var(--mk-surface,#fff);border-left:1px solid var(--mk-border,#e4e9f0);box-shadow:-12px 0 35px rgba(20,35,28,.10)}
      .mkux-head{display:flex;align-items:center;justify-content:space-between;gap:14px;padding:18px 22px;border-bottom:1px solid var(--mk-border,#e4e9f0);background:#fff}.mkux-head h2{margin:0;font-size:21px}.mkux-body{padding:20px 22px;overflow:auto;flex:1}.mkux-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.mkux-grid .wide{grid-column:1/-1}.mkux-field{display:flex;flex-direction:column;gap:6px;font-size:12px;font-weight:700}.mkux-field input,.mkux-field select,.mkux-field textarea{width:100%;box-sizing:border-box;padding:11px 12px;border-radius:11px;border:1px solid rgba(127,127,127,.3);background:#fff;color:inherit;outline:none}.mkux-field input:focus,.mkux-field select:focus,.mkux-field textarea:focus{border-color:#55a77d;box-shadow:0 0 0 3px rgba(38,146,91,.1)}.mkux-actions{display:flex;justify-content:flex-end;gap:10px;padding:14px 22px;border-top:1px solid var(--mk-border,#e4e9f0);background:#fafcfb}.mkux-toast{position:fixed;right:22px;bottom:22px;z-index:100000;padding:13px 17px;border-radius:13px;background:#171a21;color:#fff;box-shadow:0 12px 35px rgba(0,0,0,.25);animation:mkuxin .18s ease-out}.mkux-section{grid-column:1/-1;margin-top:5px;font-size:11px;text-transform:uppercase;letter-spacing:.08em;opacity:.6}.mkux-loading{display:flex;align-items:center;gap:10px;padding:12px 0}.mkux-dot{width:8px;height:8px;border-radius:50%;background:currentColor;animation:mkuxpulse 1s infinite alternate}@keyframes mkuxin{from{opacity:0;transform:translateX(8px)}to{opacity:1;transform:none}}@keyframes mkuxpulse{to{opacity:.25}}
      @media(max-width:900px){#mkUxLayer{inset:120px 0 0}.mkux-dialog{width:100%}}
