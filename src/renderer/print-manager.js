(() => {
  ...
  const printOrder = id => {
    ...
    requestAnimationFrame(() => {
      ...
      setTimeout(() => {
        ...
        area.innerHTML = '';
        const next = read().map(x => x.id === id ? { ...x, printStatus: 'printed', printedAt: new Date().toISOString() } : x);
        write(next);
        renderManager();
      }, 300);
    });
  ...
  window.addOrderToPrintQueue = () => {
    ...
    if (!order) return alert('Select an order first.');
  ...
  window.addManualPrintOrder = () => {
    ...
    if (!itemText || total <= 0) return alert('Enter items and a total greater than zero.');
