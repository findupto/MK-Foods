(() => {
  ...
  const permissions = {
    Owner: ['*'], Admin: ['*'], Manager: ['pos','orderflow','tables','kds','delivery','customers','menu','inventory','suppliers','expenses','staff','counters','printers','printmanager','banking','reports','history','transactions','accounts','settings','cleaning'],
  ...
  const defaultTasks={Dishwasher:['Wash and sanitize dish pit','Clean filters and screens','Check drain flow','Check wash/rinse temperatures','Refill detergent and rinse aid','Sweep and mop dish area','Sanitize sinks and work surfaces','Report equipment issue'],Manager:['Opening walk-through','Verify cash drawer and staff','Check printer and KDS status','Review open orders','Approve pending exceptions','Closing sign-off'],Waiter:['Check tables','Restock service station','Confirm specials','Review open tables'],Kitchen:['Sanitize prep surfaces','Check prep stock','Review KDS queue','Clean station between rushes']};
